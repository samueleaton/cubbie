...
const createStore = (configObj = {}) => (() => {
  ...
  */
  function modifyState(func) {
    const tempState = _.cloneDeep(getCurrentState());
    func(tempState);
  ...
  */
  function purgeStore() {
    const removedStates = generateFriendlyStateHistory(states).splice(0, states.length - 1);
    states.splice(0, states.length - 1);
    if (states.length)
      states[0].isInitialState = true;
    eventEmitter.emit('STORE_PURGED');
