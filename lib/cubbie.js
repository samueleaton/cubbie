...
function revertChanges(sourceObj, diffs) {
...
const createStore = (configObj = {}) => (() => {
  ...
  */
  function revertState(nTimes) {
    ...
    if (states.length > 1) {
      ...
      if (states.length > 1)
        states[states.length - 1].state = revertChanges(curr.state, prev.state);
    }
    else
      return false;
  ...
  */
  function revertStateWhere(revertCb) {
    // this shouldn't get whole client history at once, but instead as needed
  ...
  */
  function getPreviousState() {
    if (states.length <= 1)
      ...
      return revertChanges(curr.state, prev.state);
    }
  }
  ...
  */
  function getClientFacingStateHistory() {
    const history = [];
    _.forEachRight(states, (stateObj, i) => {
      if (i > 0 && i < states.length - 1)
        history.unshift(revertChanges(history[0], stateObj.state));
      else
  ...
  */
  const storeMethods = {
    ...
    clean() {
      cleanStore(states, getClientFacingStateHistory());
      return this;
    }
  };
